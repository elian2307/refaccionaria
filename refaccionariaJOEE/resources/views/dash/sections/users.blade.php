            ...
            <div class="modal-content">
                ...
                <div class="modal-grid">
                    ...
                    <p>
                        ...
                        <span class="{{ $user->is_premium ? 'suc' : 'sec' }}">
                            {{ $user->is_premium ? 'Activated' : 'Inactive' }}
                        </span>
                    </p>
                    <p><strong>Joined at:</strong> {{ $user->created_at?->format('Y-m-d') ?? 'No date' }}</p>
                    <p>
                        <strong>Addresses:</strong>
                        <span class="sec" onclick="openModal('addressesModal_{{ $user->id }}')" style="cursor:pointer;">
                            Show
                        </span>
                    </p>
                </div>
                ...
                <form class="edit-user-form" id="formEdit_{{ $user->id }}">
                    ...
                    <div class="form-group">
                        ...
                        <select name="tipo_usuario" class="edit-input eis">
                            <option value="vendedor" {{ $user->tipo_usuario == 'vendedor' ? 'selected' : '' }}>Seller</option>
                            <option value="admin" {{ $user->tipo_usuario == 'admin' ? 'selected' : '' }}>Admin</option>
                            <option value="usuario" {{ $user->tipo_usuario == 'usuario' ? 'selected' : '' }}>User</option>
                        </select>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" name="reputacion" value="{{ $user->reputacion }}" step="0.1" min="0" max="5" class="edit-input">
                    </div>
                    ...
                    <!-- 🔥 AQUÍ ESTÁ LA MEJORA -->
                    <div id="userEditErrors_{{ $user->id }}" style="margin: 1rem 0;"></div>
